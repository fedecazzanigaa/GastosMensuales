...
// Pedir permiso para notificaciones (no disponible en Safari iOS)
...
```
    ...
    if (payload.eventType === 'INSERT' && payload.new.user_id !== currentUser.id) {
      ...
      // FIX iOS: verificar que Notification existe antes de usarla
...
return `<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid var(--border)"> <span class="tarjeta-badge" style="background:${color};color:white">${t}</span> <span style="font-size:13px;font-weight:600">${montos.join(' + ')}/mes</span> </div>`;
}).join(’’) +
...
el.innerHTML =
`<div class="ms-item" onclick="msCatToggle('__all__')"> <div class="ms-check ${msCatSel.size === 0 ? 'on' : ''}"></div> <span style="font-weight:600">Todas</span> </div>` +
items.map(item =>
`<div class="ms-item" onclick="msCatToggle('${item}');event.stopPropagation()"> <div class="ms-check ${msCatSel.has(item) ? 'on' : ''}"></div> <span>${item}</span> </div>`
).join(’’);
...
el.innerHTML =
`<div class="ms-item" onclick="msPerToggle('__all__')"> <div class="ms-check ${msPerSel.size === 0 ? 'on' : ''}"></div> <span style="font-weight:600">Todos</span> </div>` +
items.map(item =>
`<div class="ms-item" onclick="msPerToggle('${item}');event.stopPropagation()"> <div class="ms-check ${msPerSel.has(item) ? 'on' : ''}"></div> <span>${item}</span> </div>`
).join(’’);
...
f.map(g => `<div class="tx-item"> <div class="tx-dot" style="background:${catColor(g.categoria)}33">${catEmoji(g.categoria)}</div> <div class="tx-info"> <div class="tx-desc">${g.descripcion || g.categoria}</div> <div class="tx-meta">${g.categoria} · ${personaBadge(g.persona)}${g.notas ? `<br><span style="font-size:10px">${g.notas}</span>` : ‘’}</div>
</div>
<div class="tx-right">
<div class="tx-amount">${fmtGasto(g.monto, g.moneda)}</div>
<div class="tx-date">${fdate(g.fecha)}</div>
...
return `<div class="cat-cfg-item"> <span class="tarjeta-badge" style="background:${TARJETA_COLORS[t]};color:white;width:80px;justify-content:center">${t}</span> <div style="flex:1;display:flex;align-items:center;gap:8px;justify-content:flex-end"> <span style="font-size:12px">Cierra día:</span> <input type="number" value="${cfg.dia_cierre}" min="1" max="31"  onchange="saveTarjetaConfig('${t}', this.value)"  style="width:50px;padding:4px;font-size:12px;text-align:center"> </div> </div>`;
}).join(’’);
}
...
return `<div class="resumen-tarjeta"> <span style="display:flex;align-items:center;gap:8px"> <span class="tarjeta-badge" style="background:${color};color:white">${t}</span> <span style="font-size:12px;color:var(--text2)">${info.count} compra${info.count>1?'s':''}</span> </span> <span style="font-size:14px">${montos.join(' + ')}/mes</span> </div>`;
}).join(’’) + ‘</div>’;
}
...
‘<div class="bar-chart">’ +
mesesData.map(m => ` <div class="bar-col"> <div class="bar-col-val">${m.total > 0 ? '$'+Math.round(m.total/1000)+'k' : ''}</div> <div class="bar-col-bar" style="height:${Math.max(m.total/maxVal*100,2)}px;background:${m.total>0?'var(--blue)':'var(--border)'}"></div> <div class="bar-col-lbl">${m.label}</div> </div>`).join(’’) +
‘</div>’;
} else {
...
return `<div class="deu-item" style="${terminada?'opacity:0.5':''}"> <div class="deu-ico" style="background:${color}22">💳</div> <div class="deu-info"> <div class="deu-desc">${d.descripcion||''}</div> <div class="deu-meta"> <span class="tarjeta-badge" style="background:${color};color:white">${d.tarjeta}</span> · <span style="color:${venc.color};font-weight:700;font-size:10px">${venc.label}</span> <br>${d.persona} · ${inicioLabel} ${d.notas ? `<br><span style="font-size:10px">${d.notas}</span>`: ''} </div> <div class="deu-progress"><div class="deu-progress-fill" style="width:${pct}%;background:${color}"></div></div> <div style="font-size:10px;color:var(--text2);margin-top:3px">${pagas}/${total} cuotas pagadas</div> </div> <div class="deu-right"> <div class="deu-monto">${fmtDeuda(d.monto_cuota, d.moneda)}</div> <div class="deu-cuota" style="font-size:10px">Total: ${fmtDeuda(d.monto_total, d.moneda).split('<br>')[0]}</div> ${!terminada ?`<button class="btn btn-sm" onclick="pagarCuota('${d.id}')" style="margin-top:6px;padding:4px 8px;font-size:11px">✓ Pagar cuota</button>` : '<div style="font-size:11px;color:var(--green);margin-top:4px;font-weight:600">✓ Pagado</div>'} <button class="btn btn-danger btn-sm" onclick="deleteDeuda('${d.id}')" style="margin-top:4px;padding:3px 7px;font-size:11px">🗑</button> </div> </div>`;
...
if (!el) return;
el.innerHTML =
`<div class="ms-item" onclick="msMetCatToggle('__all__')"> <div class="ms-check ${msMetCatSel.size === 0 ? 'on' : ''}"></div> <span style="font-weight:600">Todas</span> </div>` +
items.map(item =>
`<div class="ms-item" onclick="msMetCatToggle('${item}');event.stopPropagation()"> <div class="ms-check ${msMetCatSel.has(item) ? 'on' : ''}"></div> <span>${item}</span> </div>`
).join(’’);
...
document.getElementById(‘met-stats-grid’).innerHTML = `<div class="metric"> <div class="metric-label">Gasto Mes Actual ${msMetCatSel.size > 0 ? '(Filtrado)' : ''}</div> <div class="metric-value">${fmt(totalCurrent)}</div> <div style="font-size:10px; color:${diffColor}; font-weight:700; margin-top:4px">${diffText}</div> </div> <div class="metric"> <div class="metric-label">Promedio (Últ. 3 meses)</div> <div class="metric-value">${fmt(avg3)}</div> </div>`;
...
document.getElementById(‘met-top-cats’).innerHTML = top.map(([cat, val]) => `<div class="cat-row"> <div class="cat-row-head"> <span class="cat-name"><span class="cat-dot" style="background:${catColor(cat)}"></span>${cat}</span> <span class="cat-val">${fmt(val/3)}/mes</span> </div> <div class="bar-bg"> <div class="bar-fill" style="width:${(val / (top[0][1] || 1) * 100)}%;background:${catColor(cat)}"></div> </div> </div>`).join(’’) || ‘<div class="empty">Sin datos suficientes</div>’;
}
...
return `<div class="rec-item"> <div class="tx-dot" style="background:${catColor(r.categoria)}22">${catEmoji(r.categoria)}</div> <div class="rec-info"> <div class="rec-name">${r.descripcion}</div> <div class="rec-meta">${r.categoria} · ${personaBadge(r.persona)} · <strong>${r.moneda === 'USD' ? 'U$D ' : '$'}${parseFloat(r.monto).toLocaleString('es-AR')}</strong></div> </div> <div class="rec-actions"> ${yaCargado  ? '<span style="color:var(--green);font-size:12px;font-weight:700;margin-right:8px">Cargado ✓</span>' :`<button class="btn btn-primary btn-sm" onclick="cargarGastoRecurrente('${r.id}')">Cargar</button>` } <button class="btn btn-danger btn-sm" onclick="deleteRecurrente('${r.id}')">🗑</button> </div> </div>`;
}).join(’’);
}
...
document.getElementById(‘bal-summary’).innerHTML = ` <div class="metric"> <div class="metric-label">Ingresos</div> <div class="metric-value g">${fmt(totalIng)}</div> </div> <div class="metric"> <div class="metric-label">Gastos</div> <div class="metric-value r">${fmt(totalGast)}</div> </div> <div class="metric" style="grid-column:1/-1"> <div class="metric-label">Balance Neto (Sobrante)</div> <div class="metric-value ${neto >= 0 ? 'g' : 'r'}">${fmt(neto)}</div> </div>`;
document.getElementById(‘bal-ingresos-list’).innerHTML = ingMes.length
? ingMes.map(i => ` <div class="tx-item"> <div class="tx-dot" style="background:var(--green)22">💰</div> <div class="tx-info"> <div class="tx-desc">${i.descripcion}</div> <div class="tx-meta">${fdate(i.fecha)}</div> </div> <div class="tx-right"> <div class="tx-amount" style="color:var(--green)">${fmtGasto(i.monto, i.moneda)}</div> <button class="btn btn-danger btn-sm" onclick="deleteIngreso('${i.id}')" style="margin-top:4px;padding:2px 6px">🗑</button> </div> </div>`).join(’’)
...
return ` <div class="deu-card"> <div style="display:flex;justify-content:space-between;margin-bottom:8px"> <span style="font-weight:700">${m.descripcion}</span> <span style="font-size:12px;color:var(--text2)">${pct}%</span> </div> <div class="deu-progress"><div class="deu-progress-fill" style="width:${pct}%;background:var(--green)"></div></div> <div style="display:flex;justify-content:space-between;margin-top:8px;font-size:12px"> <span>${fmtGasto(m.monto_actual, m.moneda)}</span> <span style="color:var(--text3)">Meta: ${fmtGasto(m.monto_objetivo, m.moneda)}</span> </div> <div style="display:flex;gap:8px;margin-top:12px"> <button class="btn btn-sm" style="flex:1" onclick="updateMetaMonto('${m.id}')">Actualizar</button> <button class="btn btn-danger btn-sm" onclick="deleteMeta('${m.id}')">🗑</button> </div> </div>`;
}).join(’’)
